...
class Pc_record extends My_Controller {
    ...
    function record_list()    // 앱에서 사용
    {
        ...
        $max_value = $this->Pc_record_model->getMaxData($email);

        log_message('debug',print_r($max_value,TRUE));
        echo json_encode(array("chartData"=>$chartdata, "result"=>$result, "max_value"=>$max_value),JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES);
