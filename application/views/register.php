  <article class="container">
            ...
            <div class="col-sm-10 col-md-offset-1 col-xs-10 col-xs-offset-1">
                <form action="<?=base_url("auth/register")?>" method="post" class="form-inline">
                <!-- <form role="form" action="<?=base_url("auth/register")?>" method="post"> -->
                    <div class="form-group">
                        <label for="inputName">성명</label><?=form_error('nickname'); ?>
                        <input type="text" class="form-control" id="nickname" name="nickname" placeholder="이름을 입력해 주세요"  value="<?=set_value('nickname'); ?>">
                    </div>
                    <div class="form-group">
                        <label for="InputEmail">아이디</label><?=form_error('email'); ?>
                        <input type="text" class="form-control" id="email" name="email" placeholder="자기만의 아이디를 입력해주세요"  value="<?=set_value('email'); ?>">
                    </div>
                    <div class="form-group">
                        <label for="inputPassword">비밀번호</label><?=form_error('password'); ?>
                        <input type="password" class="form-control" id="password" name="password" placeholder="비밀번호(4자리 이상)를 입력해주세요"  value="<?=set_value('password'); ?>">
                    </div>
                    <div class="form-group">
                        <label for="inputPasswordCheck">비밀번호 확인</label><?=form_error('re_password'); ?>
                        <input type="password" class="form-control" id="re_password" name="re_password" placeholder="비밀번호 확인을 위해 다시한번 입력"  value="<?=set_value('re_password'); ?>">
                    </div>
                    <div class="form-group">
